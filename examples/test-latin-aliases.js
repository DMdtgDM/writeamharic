...
console.log('\n=== TEST 4: CRITICAL — a variable name starting with a keyword prefix must NOT falsely trigger ===');
{
  ...
  check('kahonemore = 5 stays a plain assignment, not "if"', js.includes('let kahonemore = 5;') && !js.includes('if ('));
}
{
  // same idea with ጨርስ/chers
  const { js } = transpileJS('chersstuff = 1');
  check('chersstuff = 1 stays a plain assignment, not "}"', js.includes('let chersstuff = 1;') && !js.trim().startsWith('}'));
}
{
  ...
  check('bare "be = 10" still assigns correctly (foreachLine pattern does not match a plain assignment)', js.includes('let be = 10;'));
}
